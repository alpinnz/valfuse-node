...
**This ADR is partially superseded by a role pivot.** The "core is orchestration only, with no schema/validation" architectural rule from §3 still holds — but the _role_ of `core` is no longer "placeholder for future orchestration logic." It is now the **umbrella entry point** for the entire valfuse-node ecosystem.

### Why pivot?

ADR-001's "no schema/validation in core" rule was sound: domain logic belongs in `form`, not `core`. But ADR-001 did not define what `core` _should_ export. The interim state — an empty interface and a 0-byte ESM bundle — was unacceptable to publish.
