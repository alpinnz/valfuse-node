...
describe("array rule: required", () => {
  ...
  it("fails for a non-array value (string)", () =>
    expect(validateArrayRule("not-an-array", rule)).toEqual(ERR));
  it("fails for a non-array value (number)", () =>
    expect(validateArrayRule(42, rule)).toEqual(ERR));
  it("fails for a non-array value (object)", () =>
  ...
  it("passes for an array of strings", () =>
    expect(validateArrayRule(["a", "b"], rule)).toBeNull());
});
...
describe("array rules integration via validateSchema", () => {
  it("validates required array field", () => {
    const schema = createSchema({
      tags: {
        type: "array",
        rules: [{ name: "required", error: { message: "Tags are required" } }],
      },
    });
  ...
  it("validates min array length", () => {
    const schema = createSchema({
      skills: {
        type: "array",
        rules: [{ name: "min", value: 1, error: { message: "At least 1 skill required" } }],
      },
    });
    expect(validateSchema(schema, { skills: [] }).skills?.message).toBe(
  ...
  it("validates max array length", () => {
    const schema = createSchema({
      roles: {
        type: "array",
        rules: [{ name: "max", value: 5, error: { message: "Max 5 roles allowed" } }],
      },
    });
    expect(validateSchema(schema, { roles: [1, 2, 3, 4, 5, 6] }).roles?.message).toBe(
      "Max 5 roles allowed"
    );
  ...
  it("validates exact array length", () => {
    const schema = createSchema({
      coords: {
        type: "array",
        rules: [
          { name: "length", value: 2, error: { message: "Must have exactly 2 coordinates" } },
        ],
      },
    });
    expect(validateSchema(schema, { coords: [1] }).coords?.message).toBe(
      "Must have exactly 2 coordinates"
    );
    expect(validateSchema(schema, { coords: [1, 2, 3] }).coords?.message).toBe(
      "Must have exactly 2 coordinates"
    );
  ...
  it("validates nonempty array", () => {
    const schema = createSchema({
      items: {
        type: "array",
        rules: [{ name: "nonempty", error: { message: "Cart cannot be empty" } }],
      },
    });
  ...
  it("chains nonempty + min rules on array", () => {
    ...
    expect(validateSchema(schema, { members: [] }).members?.message).toBe(
      "Must have at least one member"
    );
    // 1 element → nonempty passes, min fails
    expect(validateSchema(schema, { members: ["a"] }).members?.message).toBe(
